...
class PbpStatsFetcher:
    ...
    def resolve_nba_game_id(self, local_id: Any, sample_row: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        # Prefer stats.nba.com scoreboardv2 by date. It is much less expensive than
        # pbpstats' full-season data.nba.com schedule lookup and avoids repeated 403s
        # on old seasons in Codespaces.
        ...
        # Fallback to pbpstats season schedule only if scoreboard lookup fails.
        ...
        try:
            ...
            if scoreboard_error:
                raise RuntimeError(f"scoreboardv2 failed: {scoreboard_error}; data.nba schedule failed: {schedule_error}")
            raise
