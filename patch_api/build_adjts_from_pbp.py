...
def fetch_pbpstats_game_logs(year: int, player_ids: List[str], sleep: float, force: bool) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # Try one all-player call first. Some pbpstats API routes support it; if not, use per-player.
    all_params = {"Season": season_label, "SeasonType": "Playoffs", "EntityType": "Player"}
    try:
        payload = fetch_json_cached(API_BASE, all_params, api_cache_path(year, None, all_players=True), sleep=sleep, force=force)
        rows = find_list_of_dicts(payload)
        if rows:
            for row in rows:
                pid = str(row_get(row, ["EntityId", "PlayerId", "PlayerID", "player_id", "Player Id"]) or "").strip()
                pid = re.sub(r"\D", "", pid)
                if pid:
                    by_player[pid].append(row)
            if sum(len(v) for v in by_player.values()) > 0:
                print(f"pbpstats.com API all-player logs for {year}: {sum(len(v) for v in by_player.values())} rows")
                return by_player
    except Exception as exc:  # noqa: BLE001
        print(f"pbpstats.com all-player call not usable for {year}; falling back to per-player: {exc}")
...
def write_ingredients_csv(data: Dict[str, Any], years: Optional[List[int]], force: bool, sleep: float, limit_games: Optional[int]) -> Path:
...
def main() -> int:
    ...
    if not args.apply_only:
        write_ingredients_csv(data, years=years, force=args.force, sleep=args.sleep, limit_games=args.limit_games)
