...
## Verified game/series AdjTS from play-by-play

The compact offline build intentionally does **not** fake game-level adjusted true shooting. Box-score game rows have PTS/FGA/FTA/TOV, but not the full ingredient set needed for verified AdjTS.

A new reconstruction script is included:

```bash
python3 build_adjts_from_pbp.py --years 2025 --apply
```

For a full historical rebuild where NBA Stats play-by-play is reachable:

```bash
python3 build_adjts_from_pbp.py --years 2001-2026 --apply --sleep 0.75
```

The script fetches NBA `playbyplayv3`, caches each game in `data/pbp_cache/`, writes `data/player_game_adjts_ingredients.csv`, then updates `data-package.json`, `data/data-package.json`, and the embedded data inside `index.html`.
...
`Z Bounds` are implemented as self offensive rebounds: the player misses a field goal and then gets credited with the immediate offensive rebound of that same miss. Game/series AdjTS remains blank unless the play-by-play reconstruction is actually run and the required ingredients are available.
